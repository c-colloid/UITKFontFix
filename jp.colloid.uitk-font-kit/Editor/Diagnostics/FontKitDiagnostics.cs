...
namespace Colloid.UitkFontKit
{
    ...
    /// diagnostics window (Window > UITK Font Kit > Diagnostics) and
    ...
    /// down further.
    /// </summary>
    public static class FontKitDiagnostics
    {
        /// <summary>Builds the full report (ASCII, one finding per line).</summary>
        public static string BuildReport()
        {
            var sb = new StringBuilder(2048);
            sb.Append("UITK Font Kit diagnostics\n");
            sb.Append("package: jp.colloid.uitk-font-kit\n");
        ...
        private static void AppendResolutionSection(StringBuilder sb)
        {
            ...
            try
            {
                Font mono = FontKit.EditorMonoFont;
                sb.Append("mono   : ")
                    .Append(EmptyAsNone(FontKit.EditorMonoFontSource))
                    .Append(" (")
            ...
            try
            {
                UnityEngine.TextCore.Text.FontAsset cjk = FontKit.CjkUiFontAsset;
                sb.Append("cjk-ui : ")
                    .Append(EmptyAsNone(FontKit.CjkUiFontSource))
                    .Append(" (")
        ...
        private static void AppendEnvironmentSection(StringBuilder sb)
        {
            ...
            try
            {
                ...
                sb.Append("language   : ").Append(lang.ToString())
                    ...
                    .Append(FontKit.ShouldPreferCjkUi(lang) ? "yes" : "no")
                    .Append(")\n");
            }
            catch (System.Exception e)
            {
        ...
        private static void AppendKnownTrapsSection(StringBuilder sb)
        {
            ...
            sb.Append("* Strip U+FE0F/U+FE0E from model/user text"
                + " (FontKit.StripVariationSelectors) and keep fixed UI"
