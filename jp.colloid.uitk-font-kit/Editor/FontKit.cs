...
{
    /// <summary>
    /// Editor-side facade of UITK Font Kit: resolves a safe monospace
    ...
    public static class FontKit
    {
        ...
        /// FontKitSettings.CjkUiFontNames is installed/creatable (callers
        ...
        /// when FontKitSettings values actually change; also useful from
        /// tests.
        /// </summary>
        ...
        private static Font ResolveMono(out string source, out bool owned)
        {
            ...
            string[] paths = FontKitSettings.EditorMonoFontPaths;
            ...
            string[] names = FontKitSettings.OsMonoFontNames;
        ...
        {
            string[] installed = InstalledOsFontNames();
            string[] names = FontKitSettings.CjkUiFontNames;
            for (int i = 0; i < names.Length; i++)
            {
                string name = names[i];
                if (!ContainsIgnoreCase(installed, name))
                ...
                UnityEngine.TextCore.Text.FontAsset asset =
                    FontShims.TryCreateOsFontAsset(name, FontKitSettings.CjkUiStyleName);
                if (asset != null)
                {
