...
namespace Colloid.UitkFontKit.Tests
{
    ...
    /// </summary>
    public class FontKitResolveTests
    {
        [SetUp]
        public void SetUp()
        {
        ...
        public void EditorMonoFont_ResolvesNonNull_InBatchMode()
        {
            ...
            Assert.IsNotNull(font,
                "FontKit must always resolve a mono font (bundled TTF"
        ...
        {
            Assert.AreSame(FontKit.EditorMonoFont, FontKit.EditorMonoFont);
        }
        ...
        public void EditorMonoFontSource_ReportsWinningCandidate()
        {
            ...
            // 2022.3).
            Debug.Log("[FontKitResolveTests] mono source = " + source
                + ", font = " + FontKit.EditorMonoFont.name);
        }
        ...
        public void EditorBundledMonoFontPath_Resolves()
        {
            ...
            // and FontKitDefaults.EditorMonoFontPaths needs a new entry.
            bool any = false;
            string[] paths = FontKitDefaults.EditorMonoFontPaths;
            for (int i = 0; i < paths.Length; i++)
            {
                var font = EditorGUIUtility.Load(paths[i]) as Font;
                Debug.Log("[FontKitResolveTests] EditorGUIUtility.Load(\""
                    + paths[i] + "\") -> " + (font != null ? font.name : "null"));
                any |= font != null;
            }
            Assert.IsTrue(any,
                "no editor-bundled mono font path resolved; update"
                + " FontKitDefaults.EditorMonoFontPaths for this editor"
                + " version");
        }
        ...
        public void CjkUiFontAsset_NeverThrows_InBatchMode()
        {
            ...
            {
                UnityEngine.TextCore.Text.FontAsset unused = FontKit.CjkUiFontAsset;
            });
        }
        ...
        {
            Assert.AreSame(FontKit.CjkUiFontAsset, FontKit.CjkUiFontAsset);
        }
        ...
        public void CjkUiFontAsset_ResolvesOnWindows_AndReportsWinner()
        {
            ...
            UnityEngine.TextCore.Text.FontAsset asset = FontKit.CjkUiFontAsset;
            ...
            string source = FontKit.CjkUiFontSource;
            Assert.IsNotEmpty(source, "a candidate must have resolved");
            StringAssert.StartsWith("osasset:", source);
            Debug.Log("[FontKitResolveTests] CJK UI source = " + source
                + ", asset = " + asset.name);
        }
    }
}
